...
# Multi-task design optimization with the point cloud autoencoder
class MultiTaskOpt:
    # Assortative Mating
    def assort_mating(P, popsize, skillf, fit, rmp, eta):
        ''' Assortative mating algorithm
        Input:
          - P: Array with the design population represented in the unified
          search space. Type: <array (-1,Lz)>
          - popsize: Target population size (lambda). Type: <int>
          - skillf: Array with the skill factor assigned to each individual in
          the population
        Output:
          -
        '''

        # Probability for selecting the parents
        prob_parent = fit**2/np.sum(fit**2)
        # Size of the offspring population
        offspring = np.zeros((popsize, P.shape[1]))
        # Parent fingerprint
        par_off = np.zeros((popsize, 2))
    
        u = np.random.random(P.shape[1])*0.990
        beta = np.zeros(u.shape)
        beta[u<0.5] = (2*u[u<0.5])**(1/(eta+1))
        beta[u>=0.5] = (0.5/(1-u[u>=0.5]))**(1/(eta+1))
    
        ind = 0
        while ind < popsize:
            # Select parents
            isel_parents = np.random.choice(list(range(P.shape[0])), 
                                         2, p=prob_parent, replace=False)
            pa = P[isel_parents[0],:]
            pb = P[isel_parents[1],:]
    
            # X-over case
            if skillf[isel_parents[0]] == skillf[isel_parents[1]] or \
                                              np.random.random(1) < rmp:
                ## Offspring 1
                # Features from A
                # SBX
                offspring[ind, :] = 0.5*((beta)*pa + (2-beta)*pb)
                offspring[ind, :] *= np.random.normal(loc=1, scale=0.015, 
                                                      size=pb.shape[0])
                # Identify parents
                par_off[ind,:] = [skillf[isel_parents[0]], 
                                  skillf[isel_parents[1]]]
                ind += 1
                
                if ind == popsize: break
    
                ## Offspring 2
                # SBX
                offspring[ind, :] = 0.5*((2-beta)*pa + (beta)*pb)
                offspring[ind, :] *= np.random.normal(loc=1, scale=0.015,
                                                      size=pb.shape[0])
                # Identify parents
                par_off[ind,:] = [skillf[isel_parents[1]], 
                                  skillf[isel_parents[0]]]
    
            # Pure mutation case
            else:
                # Offspring 1
                # Mutation
                unif_mut_index = np.random.normal(loc=1, scale=0.035, 
                                                  size=pa.shape[0])
                offspring[ind, :] = pa*unif_mut_index
                # Indentify parent
                par_off[ind, :] = [skillf[isel_parents[0]], 
                                   skillf[isel_parents[0]]]
                ind += 1
                ...
                unif_mut_index = np.random.normal(loc=1, scale=0.035, 
                                                  size=pb.shape[0])
                offspring[ind, :] = pb*unif_mut_index
                # Identify parent
                par_off[ind, :] = [skillf[isel_parents[1]], 
                                   skillf[isel_parents[1]]]
    
            ind += 1
        return(offspring, par_off)

    # Vertical Cultural Transmission (selective immitation)
    def vert_cult_transm(par_fact, ntasks):
        ''' Assortative mating algorithm
        Input:
          - 
        Output:
          -
        '''
        task_off = np.zeros(par_fact.shape[0], dtype=int)
        for ind in range(par_fact.shape[0]):
            if not par_fact[ind, 0] == par_fact[ind, 1]:
                # task_off[ind] = par_fact[ind, 0]
                randchoice = np.random.random(1)
                if randchoice < 0.5:
                    task_off[ind] = par_fact[ind, 0]
                else:
                    task_off[ind] = par_fact[ind, 1]
            else:
                task_off[ind] = par_fact[ind, 0]

        # Check if the population is assigned to a single task
        if np.min(task_off) == np.max(task_off):
            randindv = np.random.choice(list(range(task_off.shape[0])), 
                                             int(task_off.shape[0]/2),
                                        replace=False)
            randtask = np.random.choice(list(range(ntasks)), 
                                             randindv.shape[0], 
                                        replace=True)
            task_off[randindv] = randtask
        return(task_off)
